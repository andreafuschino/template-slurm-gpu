...
def test_valid_dist(dataloader, model, distNet, loss_fn):
  with torch.no_grad():
    model.eval()
    distNet.eval()
    val_loss = 0
...
def trainDistanceModule(model, distNet, loss_func, device, train_dataloader, optimizer, epoch, loss_history_train, loss_history_valid, scheduler):
    model.eval()
    distNet.train()
    total_loss = 0
    ...
    val_loss=test_valid_dist(valid_dataloader, model, distNet, loss_func)
